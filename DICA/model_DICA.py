...
# %%
def train(model, discriminator, train_loader, optimizer,disc_optimizer, adv_weight, epoch, device):
    ...
    for users, p_item, n_item, mask in tqdm(train_loader):
        ...
        p_score_int = p_score_int.view(-1).unsqueeze(-1)
        p_score_pop = p_score_pop.view(-1).unsqueeze(-1)
        n_score_int = n_score_int.view(-1).unsqueeze(-1)
        n_score_pop = n_score_pop.view(-1).unsqueeze(-1)
    ...
    return sum(total_loss) / len(total_loss), sum(total_DICE_loss) / len(total_DICE_loss), sum(total_adv_loss) / len(total_adv_loss), sum(total_disc_loss) / len(total_disc_loss), sum(total_KL_loss) / len(total_KL_loss)
