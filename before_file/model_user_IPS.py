...
class DICE(nn.Module):
    def __init__(self, num_users, num_items, embedding_size, blen_pop, user_pop, dis_pen, int_weight, pop_weight, gamma, device):
        ...
        self.blen_pop = blen_pop.to(device)
        self.user_pop = user_pop.to(device)
    ...
    def pop_bpr_loss(self, p_score, n_score, pop_item_p, pop_item_n, batch_user_pop, mask):
        epsilon = 1e-10
        diff = p_score - n_score
        return -torch.mean(mask * (1/batch_user_pop)*(torch.log(torch.sigmoid(diff) + epsilon)))
    ...
    def forward(self, user, item_p, item_n, mask):
        ...
        batch_user_pop = self.user_pop[user]
        
        loss_int = self.mask_bpr_loss(p_score_int, n_score_int, mask)
        loss_pop = self.mask_bpr_loss(n_score_pop, p_score_pop, mask) + self.pop_bpr_loss(p_score_pop, n_score_pop, pop_item_p, pop_item_n, batch_user_pop,~mask) # mask : o_2, ~mask : o_1
...
def train(model, train_loader, optimizer, device):
    model.train()
    total_loss = []
    for users, p_item, n_item, mask in (train_loader):
